...
async function fetchNotes(repositoryUrl, execaOptions) {
  console.log("fetchNotes", repositoryUrl);
  try {
    await execa(
      "git",
      ["fetch", "--unshallow", "--depth=30", repositoryUrl, `+refs/notes/${GIT_NOTE_REF}:refs/notes/${GIT_NOTE_REF}`],
      execaOptions
    );
  } catch {
    await execa(
      "git",
      ["fetch", "--depth=30", repositoryUrl, `+refs/notes/${GIT_NOTE_REF}:refs/notes/${GIT_NOTE_REF}`],
      {
        ...execaOptions,
        reject: false,
      }
